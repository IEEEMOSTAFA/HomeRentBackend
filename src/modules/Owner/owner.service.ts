...
const getOwnerProfileFromDB = async (userId: string) => {
  ...
  // ✅ AUTO CREATE OwnerProfile if it doesn't exist
  if (!ownerProfile) {
    console.log(`🔄 Auto-creating OwnerProfile for user: ${userId}`);
    ...
    console.log(`✅ OwnerProfile auto-created successfully for user: ${userId}`);
  }
...
const updateOwnerProfileIntoDB = async (userId: string, data: UpdateOwnerProfileInput) => {
  ...
  await getOwnerProfileFromDB(userId);
...
const createPropertyIntoDB = async (ownerId: string, data: CreatePropertyInput) => {
  ...
  const ownerProfile = await getOwnerProfileFromDB(ownerId);

  // Check if profile is complete (phone + nid required)
  if (!ownerProfile.phone || !ownerProfile.nidNumber) {
...
//       },
//     });
//     console.log(`✅ Auto-created OwnerProfile for user: ${userId}`);
//   }
...
//   await getOwnerProfileFromDB(userId);   // This will auto-create if missing
...
// const createPropertyIntoDB = async (ownerId: string, data: CreatePropertyInput) => {
//   // Auto create profile if not exists
//   await getOwnerProfileFromDB(ownerId);
